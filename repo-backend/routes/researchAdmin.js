...
router.get(
  ...
  async (req, res) => {
    try {
       ...
       * - Staff Publishing should only manage real repository items.
       * - Faculty-approved student finals live in a separate queue (source = "faculty-approved")
       *   and must NOT appear in Publishing until staff imports/uploads them.
       * - Drafts are never publishable.
       */
      filter.submissionType = "final";
      filter.source = { $nin: ["faculty-approved", "faculty-approved-used"] };
